...
class ELFSecurity(BinarySecurity):
    ...
    def relro(self) -> RelroType:
        ...
        if flags is None:
            ...
            bind_now = lief.ELF.DynamicEntryFlags.FLAG.BIND_NOW in flags

        flags_1 = self.bin.get(lief.ELF.DynamicEntry.TAG.FLAGS_1)
        if flags_1 is None:
            now = False
        else:
            now = lief.ELF.DynamicEntryFlags.FLAG.NOW in flags_1
    ...
    def has_canary(self) -> bool:
        ...
        for section in canary_sections:
            # lief >= 1.0: get_symbol returns None instead of raising lief.not_found
            if self.bin.get_symbol(section) is not None:
                return True
        return False
    ...
    def pie(self) -> PIEType:
        if self.bin.header.file_type == lief.ELF.Header.FILE_TYPE.DYN:
            # lief >= 1.0: DYNAMIC_TAGS.DEBUG became DynamicEntry.TAG.DEBUG_TAG
    ...
    def has_rpath(self) -> bool:
        # lief >= 1.0: get returns None instead of raising lief.not_found
        return self.bin.get(lief.ELF.DynamicEntry.TAG.RPATH) is not None

    @property
    def has_runpath(self) -> bool:
        return self.bin.get(lief.ELF.DynamicEntry.TAG.RUNPATH) is not None
    ...
    def is_stripped(self) -> bool:
        # TODO: hwo to reset symtab_symbols iterator for the next call to symbols() ?
        # consumes only the first symbol from iterator, saving CPU cycles
        try:
            next(iter(self.bin.symtab_symbols))
